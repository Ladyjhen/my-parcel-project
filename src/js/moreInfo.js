import moreInfoTpl from '../template/moreInfo.hbs';
import { fiveDaysData } from '../js/base/helper';

const getMoreInfoData = target => {
  const currentDay = target.dataset.day;
  const dayInfo = fiveDaysData.find(({ day }) => day === +currentDay);

  if (!dayInfo || !dayInfo.forecast) {
    console.error('Day information or forecast data is missing.');
    return;
  }
